...
**依赖出过什么"莫名其妙"的问题吗？**
有，而且是间接发现被测系统的隐患：被测系统 requirements 钉了 `fastapi==0.104.1` 但没钉 pydantic，pydantic 2.12 发布后 FastAPI 0.104 在 import 阶段直接崩（`FieldInfo.in_`）。测试框架侧的兜底是 CI 安装依赖后显式约束 `pydantic<2.12`。这里还有个细节：我最初按报错堆栈想当然约束 `<2.13`，CI 装到 2.12.5 依然崩，用二分实测才确定安全边界是 `<2.12`。教训：**约束边界要靠实验确认，不能从报错版本倒推**；且 requirements 只钉直接依赖不钉传递依赖，等于把传递依赖的升级权交给了 PyPI，重要环境要么钉全、要么配 lock 文件。
