...
**你的 Token 是怎么管理的？**
`BaseAPI.request()` 统一在请求头注入 `Authorization: Bearer <token>`，业务代码不碰请求头。验证 Token 有效性不能用 `GET /articles`——它是公开接口，无 Token 也是 200；项目里用 `PUT /articles/999999`（有效 Token 过了鉴权层返回 404 资源不存在，无效 Token 被拦返回 401），既验证了鉴权又无副作用。日志里刻意不打印 headers，避免 Token 泄露到日志文件。
